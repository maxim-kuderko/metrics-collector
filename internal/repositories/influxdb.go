...
func NewInfluxDB(v *viper.Viper) Repo {
	...
	client := influxdb2.NewClientWithOptions(v.GetString(`INFLUXDB_URL`), v.GetString(`INFLUXDB_TOKEN`), influxdb2.DefaultOptions().SetFlushInterval(v.GetUint(`INFLUXDB_FLUSH_INTERVAL`)))
